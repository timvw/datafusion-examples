...
#[test]
fn magic() {
    let o = split_glob_expression("s3://bucket/*");
    let (x,y) = o.unwrap();
    println!("{x}, {y}");

    assert!(split_glob_expression("s3://bucket/a") == None);
    assert!(split_glob_expression("s3://bucket/a*") == Some(("s3://bucket", "a*")));
}

#[tokio::test]
async fn meh() {
    let sdk_config = aws_config::load_from_env().await;
    println!("config: {:?}", sdk_config);
